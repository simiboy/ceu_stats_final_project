...
urlcolor: blue
abstract: "The conservative news channel Fox News was founded in 1996 and gradually implemented in various US cities. This led to the situation that in the year 2000 there was a presidential election in which a significant proportion of the US population already had access to this TV     channel for several years and other cities had no access at all at the time of the election. The used dataset includes among controlvaribles the vote shares for the Republicans of the Presidential election in 1992, 1996 and 2000. Futhermore the share of Fox News subscribers in 2000 among cities. The available data make it possible to ask the question to which extent had the implementation of Fox News affected the US voting behavior in the presidential elections between 1996 and 2000? The analysis yields revealing results, but the idea of the highly influential TV channel, the so-called 'FOX NEWS EFFECT', had to be revised, as the analysis showed no significance in this area. "
...
#library(usmap)
#plot_usmap(data = data.frame(state = names(gop2000_by_states), values = gop2000_by_states)) + scale_fill_continuous(low = "white", high = "red", name = "Republican vote share") +theme(legend.position = "right")
```
...
boxplot(data[,c(3, 4, 5, 6, 7)], ylab="Proportion (first four variables) / Rate of income", names = c("Collage eduacted", "Male", "Black", "Hispanic", "Average income"))
```

The histogram in (Figure \ref{fig:independentVariables}) shows the general quality of the independent variables of the data set. It is visible that the mean of male population in the towns are around 0.5, however there are more towns where there are an extremity towards a high male population share than to women. The black and Hispanic population of the towns in the study is generally low, but there are cases with especially high rate from both ethnic groups. The mean of education and the income is below 0.2 and the later one has a higher expansion.
...
ntowns <- length(unique(data$town))
...
```{r, echo=FALSE}
cordata <- cor(data[12], data[,c(3, 4, 5, 6, 7)])
...
text(1994,0.45, "Towns without Fox News")
...
```{r diffInDiffBlack, fig.cap="Difference in Average Vote Share for the Republicans With High Black Population", echo=FALSE}
##UJJ -> no Thea's
...
xlim = c(1992, 2000), ylim = c(0.0, 0.5), xlab = "Years of presidential elections",
...
text(1994, 0.45,"Towns with Fox News", col="aquamarine3")
text(1998,0.2,"Towns with some Fox News", col="darkseagreen4")
text(1993,0.2, "Towns with no Fox News", col = "mediumorchid1")
...
text(1993.5,0.12,"Low Hispanic Population", col="darkseagreen4")
text(1993.5, 0.04, "No Hispanic Population", col="mediumorchid1")
```
...
high_income.t_w_foxnews<- subset(towns_w_foxnews, subset = income1990 >= 0.228)
high_income.t_no_foxnews<- subset(towns_no_foxnews, subset = income1990 >= 0.228)
...
medium_income.t_w_foxnews<- subset(towns_w_foxnews, subset = income1990 >0.13093333333 & income1990 <0.228 )
medium_income.t_no_foxnews<- subset(towns_no_foxnews, subset =income1990 >0.13093333333 & income1990 <0.228 )
...
low_income.t_w_foxnews<- subset(towns_w_foxnews, subset = income1990 <= 0.13093333333)
low_income.t_no_foxnews<- subset(towns_no_foxnews, subset = income1990 <= 0.13093333333)
...
The results show a slight positive relationship between income and vote share for Republicans. With the increasing share of the ethic groups within the population has a negative effect on Republican vote share and education level has generally a low impact.
